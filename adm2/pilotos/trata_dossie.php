<?php
	...
	//	----------------------
	// 1. Apaga todos os registros relativos ao piloto de PILOTO_PAIS
	$sql = "delete from piloto_tetr where callsign = $callsign";
	$res = $con->query($sql);
	if ($_POST['chk_tetr']){
		// 2. Salva os dados atuais
		foreach ( $_POST['chk_tetr'] as $k=> $c){
			$sql = "insert into piloto_tetr (callsign, cod_tetr) values ($callsign, $c)";
			$res = $con->query($sql);
		}
	}
...
?>
<script>
window.open('../index.php?pagina=pilotos/pilotos','_parent')
</script>
